...
typedef std::vector<uchar> EducationalRow;

struct EducationalTable {
	std::vector<std::shared_ptr<EducationalRow>> rows;
	std::shared_ptr<std::string>> name;
};

struct EducationalDatabase {
	std::vector<std::shared_ptr<EducationalRow>> tables;
};

// Final prevents further inheritance
class ha_educational final : public handler {
	uint current_position = 0;
	std::shared_ptr<EducationalTable> educational_table = nullptr;

public:
	ha_educational(handlerton *hton, TABLE_SHARE *table_arg) {}

	~ha_educational() = default;
	...
	#define EDUCATIONAL_MAX_KEY MAX_KEY // Max allowed keys
	#define EDUCATIONAL_MAX_KEY_SEG 16 // Max segments for key
	#define EDUCATIONAL_MAX_KEY_LENGTH 3500; // Like in InnoDB
	
	uint max_supported_keys() const { return EDUCATIONAL_MAX_KEY; }

	uint max_supported_key_length() const { return EDUCATIONAL_MAX_KEY_LENGTH; }

	uint max_supported_key_part_length() const { return EDUCATIONAL_MAX_KEY_LENGTH; }
	...
	int index_read_idx_map(uchar *buf, uint idx, const uchar *key, key_part_map keypart_map, enum ha_rkey_function find_flag) {
		return HA_ERR_END_OF_FILE;
	}
